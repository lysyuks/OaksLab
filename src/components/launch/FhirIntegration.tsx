...
const risks = [
  { level: '🟡', title: 'Complexity', why: 'FHIR is technical', mit: 'Templates, auto-suggestions, training' },
  { level: '🟡', title: 'EHR API Limits', why: 'Rate limiting', mit: 'Queue system, retry logic' },
  { level: '🔴', title: 'Mapping Errors', why: 'Wrong data could affect care', mit: 'Validation, preview mode, testing' },
  { level: '🟡', title: 'Adoption', why: 'Clients may find it complex', mit: 'Make optional initially, show ROI' },
];
  ...
  return (
    <div>
      <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-3 mb-6 cursor-pointer">
        ...
        <span className="text-xs text-teal-600 ml-1 font-medium">Healthcare data exchange standard</span>
      ...
      <div className={`${expanded ? '' : 'hidden'}`}>
        <div className="space-y-8">
          ...
          {/* ── 1. What & Why ── */}
          <div className="bg-teal-50/50 border border-teal-200 rounded-2xl p-5">
            <h5 className="font-bold text-sm text-teal-800 mb-2">What is FHIR R4?</h5>
            <p className="text-sm text-slate-600 leading-relaxed mb-4">
              <span className="font-bold text-teal-700">FHIR R4 (Fast Healthcare Interoperability Resources)</span> is the healthcare industry standard for exchanging patient data between systems. It enables automatic transfer of intake form data to doctors' EHR systems (Epic, Cerner, Athena) in a standardized format.
            </p>
            <div className="flex flex-wrap gap-2">
              {benefits.map((b, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium bg-teal-100/60 text-teal-700">
                  ...
                  <div key={ep.num} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: ACCENT }}>Epic {ep.num}</span>
                          <h6 className="text-sm font-bold text-slate-800">{ep.title}</h6>
                        </div>
                      </div>
                      <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-teal-50 text-teal-700">{ep.dur}</span>
                    </div>
                    <ul className="space-y-1">{ep.items.map((it, j) => (<li key={j} className="flex items-start gap-2 text-xs text-slate-600"><span className="mt-1 w-1 h-1 rounded-full bg-teal-500 shrink-0" />{it}</li>))}</ul>
                  </div>
                ))}
              </div>
              {/* Impact */}
          ...
          <div>
            ...
            <div className="grid md:grid-cols-[1fr_auto_1fr] gap-4 items-start">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <div className="text-xs font-bold uppercase tracking-wider text-amber-600 mb-2">Patient Fills Out</div>
              ...
              <div className="hidden md:flex items-center justify-center py-8">
                <div className="flex flex-col items-center gap-1">
                  <svg className="w-8 h-8 text-teal-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                  <span className="text-[10px] font-bold text-teal-500 uppercase">Maps to</span>
                </div>
              </div>
              {/* FHIR resources */}
              <div className="bg-teal-50 border border-teal-200 rounded-xl p-4">
                <div className="text-xs font-bold uppercase tracking-wider text-teal-600 mb-2">System Creates 7 FHIR Resources</div>
                <ul className="space-y-1.5">
                  {fhirResources.map((r) => (
                    <li key={r.num} className="flex items-center gap-2 text-sm">
                      <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-white bg-teal-500">{r.num}</span>
                      <span className="font-semibold text-teal-800">{r.resource}</span>
                      <span className="text-slate-400">({r.desc})</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-3 pt-2 border-t border-teal-200 text-xs text-teal-600 font-medium">
                  All packaged in a <span className="font-bold">FHIR Bundle</span> → Sent to doctor's EHR via API
                </div>
              </div>
            </div>
            ...
            {/* Code snippet */}
            <div className="mt-3">
              <button onClick={() => setShowSnippet(!showSnippet)} className="text-xs font-medium text-teal-600 hover:text-teal-800 cursor-pointer flex items-center gap-1">
              ...
              <div className={`overflow-hidden transition-all duration-300 ${showSnippet ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                <pre className="bg-slate-900 text-slate-300 text-xs p-4 rounded-xl font-mono overflow-x-auto">{fhirSnippet}</pre>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-4">
                <div className="grid grid-cols-[1fr_auto_1fr] gap-x-3 gap-y-2 text-sm">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Form Field</span>
                  <span />
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">FHIR Resource</span>
                  {mappingRows.map((row, i) => (
                    <div key={i} className="contents">
                      <span className="py-1.5 text-slate-700 font-medium">{row.field}</span>
                      <span className="py-1.5 text-teal-400 text-center">→</span>
                      <span className="py-1.5 font-mono text-xs text-teal-700 bg-teal-50 px-2 rounded">{row.fhir}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {['Auto-suggest appropriate mappings', 'Templates for weight loss, dermatology, etc.', 'Preview FHIR output before saving'].map((f, i) => (
                <span key={i} className="text-xs px-2.5 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-200">{f}</span>
              ))}
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
              ...
              <div className="relative pl-6 space-y-0">
                <div className="absolute left-[9px] top-2 bottom-2 w-0.5 bg-teal-200" />
                {flowSteps.map((s, i) => (
                  <div key={i} className="relative flex items-start gap-3 py-1.5">
                    <div className="absolute -left-6 mt-0.5 w-[18px] h-[18px] rounded-full flex items-center justify-center text-[9px] font-bold text-white z-10" style={{ backgroundColor: ACCENT }}>{i + 1}</div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
              ...
              <div className="space-y-2.5">
                {integrationMethods.map((m, i) => (
                  <div key={i} className={`flex items-center justify-between text-sm rounded-lg p-3 ${m.rec ? 'bg-teal-50 border border-teal-200' : 'bg-slate-50 border border-slate-200'}`}>
                    <div>
                      <span className={`font-semibold ${m.rec ? 'text-teal-800' : 'text-slate-700'}`}>{m.method}</span>
                      <span className="text-slate-400 ml-2 text-xs">— {m.desc}</span>
                    </div>
                    {m.rec && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-teal-500 text-white">Recommended</span>}
                  </div>
                ))}
              </div>
            </div>
              ...
              <tbody>
                {resourceTable.map((row, i) => (
                  <tr key={i} className="border-b border-slate-50 hover:bg-teal-50/30 transition-colors">
                    ...
                    <td className="py-2.5 px-4 font-mono text-xs text-teal-700">{row.resource}</td>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 text-center">
              ...
              <div className="text-sm text-slate-500 mb-2">Before: 15 min × 100 pts = 25 hrs/month</div>
              <div className="text-sm font-bold text-emerald-600">After: 0 min (automatic)</div>
            </div>
            <div className="bg-emerald-50 rounded-xl border border-emerald-200 p-5 text-center">
              <div className="text-2xl mb-2">💰</div>
              <div className="text-xs font-bold uppercase tracking-wider text-emerald-600 mb-1">Annual Savings</div>
              <div className="text-2xl font-bold text-emerald-700">$7,500</div>
              <div className="text-xs text-emerald-600">per client per year</div>
            </div>
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 text-center">
              <div className="text-2xl mb-2">✅</div>
              ...
              <div className="text-sm font-bold text-emerald-600">After: &lt;1% errors</div>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {risks.map((r, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4" style={{ borderLeft: `3px solid ${r.level === '🔴' ? '#ef4444' : '#f59e0b'}` }}>
                ...
                <p className="text-xs text-slate-600"><span className="font-semibold text-emerald-600">Mitigation:</span> {r.mit}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-x-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3 text-xs">
                ...
                <svg className="w-4 h-4 text-teal-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                <span className="font-bold text-teal-700">With FHIR: 7.5 months</span>
              </div>
            </div>
                  ...
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 relative h-7 bg-slate-50 rounded-lg">
                      <div className="absolute top-0.5 bottom-0.5 rounded-md flex items-center justify-center" style={{ left: `${leftPct}%`, width: `${widthPct}%`, backgroundColor: p.color, opacity: 0.85 }}>
                        <span className="text-[10px] font-bold text-white/90">{p.end - p.start + 1}w</span>
                      </div>
                    </div>
                  </div>
                );
                    ...
                    <div>
                      ...
                      <ul className="mt-1 space-y-1">
                        {['Essential for enterprise clients', 'Strong ROI ($7,500/yr per client)', 'Competitive advantage', 'Regulatory compliance requirement'].map((w, i) => (
                          <li key={i} className="flex items-start gap-1.5"><span className="mt-1 w-1 h-1 rounded-full bg-white/60 shrink-0" />{w}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
